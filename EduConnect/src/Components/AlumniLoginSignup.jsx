...
export default function AlumniLoginSignup() {
...
//   const [forgotEmail, setForgotEmail] = useState("");
  ...
  const saveTokenAndRedirect = (token) => {
    ...
    navigate("/alumni"); // redirect to alumni dashboard route
  };
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      const res = await fetch("/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...formData, role: "alumni" }), // pass role
      });
      const data = await safeJson(res);
        ...
        {showSignup ? (
          ...
          <form autoComplete="off" className="auth-form" onSubmit={handleLoginSubmit}>
            ...
            <span
              ...
              style={{ fontSize: "0.9em", marginBottom: "10px", cursor: "pointer", display: "block", textAlign: "right" }}
